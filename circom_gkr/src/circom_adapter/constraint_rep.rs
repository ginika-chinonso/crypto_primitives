...
///
/// This file contains how constraints are represented
/////////////////////////////////////////////////////////
use std::vec;
...
pub struct Constraint<F: PrimeField> {
    // The usize tells us the index of the element in the witness array
    // The field element tells if it is a positive or negative value
    // Op is the operation which may be multiplication or addition
    pub a: Vec<(usize, F)>,
    pub b: Vec<(usize, F)>,
    pub c: Vec<(usize, F)>,
    pub op: Op,
...
impl<F: PrimeField> Constraint<F> {
    ...
    // If an operation is not specified, it is taken as an addition
    ...
    pub fn move_to_fill(&mut self) {
        if self.can_move() {
            ...
            // self.c = new_self.c;
            // self.op = new_self.op;
            *self = Constraint::new_without_op(&self.a, &self.b, &self.c);
        }
    }
}
