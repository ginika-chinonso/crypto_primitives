...
pub mod test {
    ...
    fn test_blind_auction() {
        ...
        let uche_reveal = auction.open(50, "Hello world".as_bytes(), uche_commitment_index).unwrap();
        println!("Uche committed: {}", uche_reveal);

        let abiodun_reveal = auction.open(100, "Hi there".as_bytes(),abiodun_commitment_index).unwrap();
